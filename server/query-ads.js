...
async function getIterationsFromADS(witAPI) {
  try {
    ...
    const queryResult = await witAPI.getClassificationNode(
      Constants.ADSConfig.PROJECT,
      'iterations',
      Constants.ADSConfig.ITERATION_OFFSET,
      2
    );
...
async function getFeatureFromADS(witAPI, id, asOfDate) {
  try {
    const queryResult = await witAPI.getWorkItem(
      id,
      [
        Constants.ADSFields.TITLE,
        Constants.ADSFields.NODENAME,
        Constants.ADSFields.ITERATION_PATH,
        Constants.ADSFields.STATE,
        Constants.ADSFields.TAGS,
        Constants.ADSFields.PRIORITY,
        Constants.ADSFields.EFFORT,
        Constants.ADSFields.RELEASE,
      ],
      asOfDate
    );
    ...
    collection.insert({
      ...
      progress: 0,
      startSprint: Constants.START_SPRINT_NOT_SET,
      endSprint: Constants.NOT_SET,
      team: teamName,
      project: projectName,
...
async function getStoriesFromADS(witAPI, pis, asOfDate) {
  try {
    ...
    for (const [i, pi] of pis.entries()) {
      piSubQuery += `[Source].[System.IterationPath] UNDER '${Constants.ADSConfig.PROJECT}${
        Constants.ADSConfig.ITERATION_OFFSET_WIQL
...
async function getFeaturesFromADS(witAPI, pis, asOfDate) {
  try {
    let piSubQuery = '';
    for (const [i, pi] of pis.entries()) {
      piSubQuery += `[System.IterationPath] UNDER '${Constants.ADSConfig.PROJECT}${
        Constants.ADSConfig.ITERATION_OFFSET_WIQL
      }\\${pi}' ${i !== pis.length - 1 ? 'OR ' : ''}`;
    }
    const asOfSubQuery = asOfDate ? `ASOF '${asOfDate}'` : '';
